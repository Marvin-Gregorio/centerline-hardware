        ...
        ?>
            <div class="row">
                <div class="col-4 d-flex align-items-center">
                    <img src="<?php echo validate_image($_settings->info('logo')) ?>" alt="" height="100px">

                    <div class="ml-3">
                        <p class="m-0 text-xs"><?php echo $_settings->info('company_name') ?></p>
                        <p class="m-0 text-xs"><?php echo $_settings->info('company_email') ?></p>
                        <p class="m-0 text-xs"><?php echo $_settings->info('company_address') ?></p>
                    </div>
                </div>
                <div class="col-4 d-flex align-items-center">
                    <h2 class="text-center"><b>SALES INVOICE</b></h2>
                </div>
                <div class="col-4">
                    <div class="row mb-2">
                        <div>
                            ...
                            $sup_qry = $conn->query("SELECT * FROM supplier_list where id = '$sup_id'");
                            $supplier = $sup_qry->fetch_array();
                            ?>
                            <div>
                                <p class="m-0 text-xs"><?php echo $supplier['name'] ?></p>
                                <p class="m-0 text-xs"><?php echo $supplier['address'] ?></p>
                                <p class="m-0 text-xs"><?php echo $supplier['contact_person'] ?></p>
                                <p class="m-0 text-xs"><?php echo $supplier['contact'] ?></p>
                                <p class="m-0 text-xs"><?php echo $supplier['email'] ?></p>
                            </div>
                        </div>
                ...
                <div class="row">
                    <div class="col-6">
                        <p  class="m-0"><b>P.O. #: <?php echo $po_no ?></b></p>
                    </div>
                    <div class="col-6">
                        <p  class="m-0"><b>Date Created : <?php echo date("Y-m-d",strtotime($date_created)) ?></b></p>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <table class="table table-striped table-bordered" id="item-list">
                            ...
                            <tbody>
                                ...
                                if(isset($id)):
                                $ord_id = $row['id'];
                                $order_items_qry = $conn->query("SELECT o.*,i.name, i.description FROM `order_items` o inner join item_list i on o.item_id = i.id where o.`po_id` = '$ord_id'");
                                $sub_total = 0;
                                ...
                                <tr class="po-item" data-id="">
                                    ...
                                    <td class="align-middle p-1 hide-item-code"><?php echo $row['name'] ?></td>
                            ...
                            <tfoot>
                                <tr class="bg-lightblue">
                                    <tr>
                                        <th class="p-1 text-right print-sub" colspan="5">Sub Total</th>
                                        <th class="p-1 text-right" id="sub_total"><?php echo number_format($sub_total) ?></th>
                                    </tr>
                                    <tr>
                                        <th class="p-1 text-right print-sub" colspan="5">Discount (<?php echo isset($discount_percentage) ? $discount_percentage : 0 ?>%)
                                        </th>
                                        <th class="p-1 text-right"><?php echo isset($discount_amount) ? number_format($discount_amount) : 0 ?></th>
                                    </tr>
                                    <tr>
                                        <th class="p-1 text-right print-sub" colspan="5">Tax Inclusive (<?php echo isset($tax_percentage) ? $tax_percentage : 0 ?>%)</th>
                                        <th class="p-1 text-right"><?php echo isset($tax_amount) ? number_format($tax_amount) : 0 ?></th>
                                    </tr>
                                    <tr>
                                        <th class="p-1 text-right print-sub" colspan="5">Total</th>
...
<script>
	$(function(){
        $('#print').click(function(e){
            $('.hide-item-code').removeClass('d-block');
            $('.hide-item-code').addClass('d-none');
            $('.print-sub').attr('colspan','4');
            e.preventDefault();
            start_loader();
            var _h = $('head').clone()
            ...
            var nw = window.open("","","width=1200,height=950")
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        end_loader();
                        nw.close()
                        $('.print-sub').attr('colspan','5');
                        $('.hide-item-code').removeClass('d-none');
                        $('.hide-item-code').addClass('d-block');
