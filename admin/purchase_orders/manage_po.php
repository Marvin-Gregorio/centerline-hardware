		...
		<form action="" id="po-form">
			...
			<?php
				if(isset($_GET['id'])){
					...
					echo '<div class="row" id="item_view" style="display:none">';
				}
			?>
				...
				<div class="col-md-12">
					<table class="table table-striped table-bordered" id="item-list">
						...
						<tbody>
							<?php 
							if(isset($id)):
							$order_items_qry = $conn->query("SELECT o.*,i.unit,i.name, i.description FROM `order_items` o inner join item_list i on o.item_id = i.id where o.`po_id` = '$id' ");
							echo $conn->error;
							...
							<tr class="po-item" data-id="">
								...
								<td class="align-middle p-1">
									<input style="pointer-events: none; cursor: not-allowed;" type="text" class="text-center w-100 border-0" name="unit[]" value="<?php echo $row['unit'] ?>" />
								</td>
								<td class="align-middle p-1">
								...
								<td class="align-middle p-1 item-price">
									<input type="number" step="any" class="text-right w-100 border-0 item-price" name="unit_price[]" value="<?=$row['unit_price']?>"/>
								</td>
	...
	function _autocomplete(_item){
		_item.find('.item_id').autocomplete({
			source:function(request, response){
				$.ajax({
					url:_base_url_+"classes/Master.php?f=search_items&location="+$('#supplier_id').val(),
					method:'POST',
					data:{q:request.term},
					dataType:'json',
					error:err=>{
