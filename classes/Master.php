...
Class Master extends DBConnection {
	...
	function save_item(){
		...
		if(empty($id)){
			...
			if(isset($_POST['restock'])){
				$sql = "UPDATE `item_list` set stock = stock + '$stock'  where id = '{$id}' ";
			}else{
	...
	function search_items(){
		...
		if(isset($_GET['location'])){
			...
			$qry = $this->conn->query("SELECT * FROM item_list where `name` LIKE '%{$q}%' AND `location` = '$location'");
		}
	...
	function save_po(){
		...
		if($save){
			...
			for($x = 0; $x < count($_POST['item_id']); $x++){
				...
				$item_qty = $_POST['qty'][$x];
				$this->conn->query("UPDATE item_list SET stock = stock - '$item_qty' WHERE id = '$item_id'");
			}

			if(empty($id))
				$this->settings->set_flashdata('success',"Purchase Order successfully added");
			else
