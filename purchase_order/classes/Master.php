...
switch ($action) {
	...
	default:
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id'])) {
			...
			// Get the input quantity and item ID from the submitted form
			$itemQuantities = $_POST['qty'];
			$itemIds = $_POST['item_id'];
		
			// Iterate through the submitted items and update the database
			for ($i = 0; $i < count($itemIds); $i++) {
				$itemId = $itemIds[$i];
				$itemQuantity = $itemQuantities[$i];
		
				// Subtract the quantity from the database (assuming you have a po_list table)
				$updateQuery = "UPDATE `po_list` SET `quantity` = `quantity` - $itemQuantity WHERE `id` = $itemId";
				$conn->query($updateQuery);
		
				// You might want to add error handling for the update query
				if ($conn->error) {
					echo json_encode(['status' => 'error', 'msg' => 'Error updating quantity in the database']);
					exit;
				}
			}
